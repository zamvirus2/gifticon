...
- 이 상태에서, 배포중 Downtime이 발생하는것을 확인하기 위해 특정버전의 Docker image로 재배포를 수행한다. (배포 수행 직후 siege 모니터링 필요)
```
...
- readiness probe 설정을 제거하였으므로, pod 배포 중 서비스가 아직 올라오지 않아 요청처리 실패하는것을 확인가능
```
